...
# Filters arthropod species, removes mites/ambiguous taxa, retains genus/species level OTUs by Ariane
...
# Applies Relative Abundance Analysis (RAA) threshold (0.3% of total reads) to filter low-abundance OTUs by Ariane
...
# Subsets interaction data by specified site types using metadata
...
# Merges sequencing data with bird/insect metadata, aggregates interactions at species level
...
# Quantifies forest/non-forest bird richness in interaction matrices
...
# Calculates avian forest dependency metrics from data
...
# Computes bipartite network metrics: pest-forest bird interaction strengths, species counts
web_feature <- function(bird_insect_web, bird_forest_coverage, insect_role_all){
  # extract from bird_insect_web
  bird_names <- colnames(bird_insect_web)
  insect_names <- rownames(bird_insect_web)
  
  # match and sum forest and non-forest bird
  ...
  # match and sum pest and non-pest
  ...
  # calculate interaction intension (sum, rather than degree)
...
# Visualizes bipartite networks with forest birds (green) and pests (red) highlighted
...
# calculate index of modularity, nestedness, specialization
# Projects bipartite interactions to unipartite bird network, calculates dietary overlap
calculate_bird_feature <- function(site_name, bird_insect_web, bird_insect_web_weighted, insect_role){
  ...
  # calculate closeness for every bird node
...
# Quantifies modular structure in bipartite networks using Walktrap algorithm
calculate_modularity <- function(bird_insect_web) {
  # transfer bird_insect_web to igraph object without weight
  ...
  # return modularity for every community
...
check_correlation <- function(y) {
  ...
  if (cor_test$p.value < 0.10) {
    ...
    print("there is not correlation")
  }
}

# try to find the relationship on beak length and diet(not found)
extract_beak_length <- function(abundance, bird_meta, bird_name_col, beak_length_col) {
  # if abundance is a vector
...
# generates a permutation of an input sequence by iteratively selecting elements through a probabilistic hierarchical splitting process
...
# Simulates sequential bird extinctions, tracks pest control resilience via interaction loss
...
Calculates Area Under Curve (AUC) for normalized extinction curves
...
plot_species_metric <- function(species_result){
  ...
  # extract four class to long data
  plot_data <- final_df %>%
  ...
  p <- ggplot(plot_data, aes(x = feature, y = value, fill = site_type)) +
    ...
    scale_fill_manual(values = c("Forest Interior" = "#238E23", "Forest Edge" = "#91A724", "Agriculture" = "darkgoldenrod2")) +
    labs(title = "Standardized Box Plot of Bird Species Features by Site Type",
         x = "Feature",
         y = "Value of Species Metrics",
       fill = "Site Type") + 
    scale_x_discrete(labels = c("Pest Dietary (PD)", "Pest Dietary (PD)", "Species Specialisation (d')", "Diet Overlap (DO)")) + 
    ...
    geom_vline(xintercept = vline_positions, linetype = "dashed", color = "grey", size = 0.7)
...
plot_network_metric <- function(basic_network_analysis) {
  ...
  # standalization(to 0-1) in every site type
  df <- df %>%
    group_by(Metric) %>%
  ...
  p <- ggplot(df, aes(x = Metric, y = StandardizedValue, fill = LandUseType)) +
    ...
    geom_vline(xintercept = seq(1.5, 5.5, by = 1), linetype = "dashed", color = "grey", size = 0.7)  
...
statistic_abundance <- function(matrix_list, bird_coverage) {
  ...
  for (i in seq_along(matrix_list)) {
    ...
    # sum bird abundance of each type
